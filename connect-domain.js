...
async function addDomainToVercel() {
  ...
  try {
    const response = await axios.post(
      `https://api.vercel.com/v10/projects/${config.PROJECT_NAME}/domains`,
      { name: config.DOMAIN },
      { 
        headers: { 
...
async function getVercelVerification() {
  console.log('Step 2: Getting verification requirements from Vercel...');
  try {
    const response = await axios.get(
      `https://api.vercel.com/v10/projects/${config.PROJECT_NAME}/domains/${config.DOMAIN}/verification`,
      { 
        headers: { 
          Authorization: `Bearer ${config.VERCEL_API_KEY}` 
        } 
    ...
    console.error('Error getting verification details:', error.response?.data || error.message);
    throw error;
  }
}

async function updateNamecheapDNS(verificationData, clientIP) {
  console.log('Step 3: Updating DNS settings in Namecheap...');
  
  // Extract verification records from Vercel
  const { verification } = verificationData;
  const txtRecords = verification.filter(record => record.type === 'TXT');
  ...
  // Build parameters for DNS update
  let params = {
  ...
  // Add TXT verification records if needed
  if (txtRecords.length > 0) {
    params.HostName3 = txtRecords[0].name === '@' ? '@' : txtRecords[0].name;
    params.RecordType3 = 'TXT';
    params.Address3 = txtRecords[0].value;
    params.TTL3 = config.TTL; // 1 minute TTL
  }
...
async function checkDomainStatus() {
  console.log('Step 4: Checking domain verification status...');
  try {
    const response = await axios.get(
...
async function connectDomain() {
  try {
    ...
    const verificationData = await getVercelVerification();
    await updateNamecheapDNS(verificationData, clientIP);
